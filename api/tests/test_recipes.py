...
def test_to_float_parses_units_and_rejects_garbage():
    from app.recipes import pdf_parser

    assert pdf_parser._to_float("270 kcal") == 270.0
    assert pdf_parser._to_float("12.5 g") == 12.5
    assert pdf_parser._to_float(None) is None
    assert pdf_parser._to_float("no numbers here") is None


@patch("app.recipes.pdf_parser.anthropic.Anthropic")
def test_parse_recipe_pdf_uses_claude(mock_anthropic_cls):
    from app.recipes import pdf_parser

    # Minimal single-page PDF with extractable text (not a real recipe page — just
    # verifies that the Claude call is made with the extracted text and the response
    # is shaped correctly). We mock extract_pdf_text so we don't need a real PDF.
    claude_response = {
        "title": "Pork Belly Bites",
        "category": "Dinner",
        ...
        "instructions": ["Sear it"],
        "fact": "Great weeknight meal.",
    }
    mock_msg = MagicMock()
    mock_msg.content = [MagicMock(text=__import__("json").dumps(claude_response))]
    mock_anthropic_cls.return_value.messages.create.return_value = mock_msg

    with (
        patch("app.recipes.pdf_parser.extract_pdf_text", return_value="Pork Belly Bites\n1 lb pork belly\nSear it"),
        patch("app.recipes.pdf_parser.get_settings") as mock_settings,
    ):
        mock_settings.return_value.anthropic_api_key = "test-key"
        result = pdf_parser.parse_recipe_pdf(b"%PDF-fake")

    assert result["title"] == "Pork Belly Bites"
    assert result["calories_kcal"] == 320.0
    assert result["ingredients"] == ["1 lb pork belly"]
    assert result["image_url"] is None


def test_parse_recipe_pdf_raises_on_empty_text():
    from app.recipes import pdf_parser

    with (
        patch("app.recipes.pdf_parser.extract_pdf_text", return_value="   "),
        patch("app.recipes.pdf_parser.get_settings") as mock_settings,
    ):
        mock_settings.return_value.anthropic_api_key = "test-key"
        try:
            pdf_parser.parse_recipe_pdf(b"%PDF-fake")
...
@patch("app.recipes.pdf_parser.parse_recipe_pdf")
def test_import_recipe_pdf_endpoint_ok(mock_parse):
...
def test_import_recipe_pdf_endpoint_rejects_non_pdf():
    app.dependency_overrides[current_user_id] = lambda: "u1"
    try:
        res = client.post(
            "/recipes/import-pdf",
            files={"file": ("recipe.txt", b"not a pdf", "text/plain")},
        )
        assert res.status_code == 422
    finally:
        app.dependency_overrides.clear()
